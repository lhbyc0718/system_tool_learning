...
\begin{enumerate}
    ...
    \item \textbf{latexmk 可用但默认调用 pdflatex}：通过命令 \texttt{latexmk -xelatex} 指定 XeLaTeX 引擎，以支持 Unicode 中文。
    \item \textbf{中文宏包缺失（xeCJK.sty 未找到）}：安装 \texttt{texlive-lang-chinese} 后，再运行 \texttt{sudo texhash} 更新文件名数据库，即可被识别。
    \item \textbf{中文字体配置}：系统默认无中文字体，安装 \texttt{fonts-noto-cjk} 后，在 LaTeX 中用 \verb+[ -d /tmp/mydir ] || mkdir /tmp/mydir && echo "创建成功或已存在"+ 等命令指定。
    \item \textbf{编译器务必选 XeLaTeX}：在 Overleaf 或本地均需切换至 XeLaTeX，否则 Unicode 字符报错。
\end{enumerate}
...
\begin{itemize}
    ...
    \item 通过 \verb+[ -d /tmp/mydir ] || mkdir /tmp/mydir && echo "创建成功或已存在"+ 指定系统字体。
    \item 公式、表格均添加 \texttt{label} 并用 \verb+[ -d /tmp/mydir ] || mkdir /tmp/mydir && echo "创建成功或已存在"+、\verb|\eqref| 引用。
\end{itemize}
示例：
\begin{verbatim}
\begin{equation}
